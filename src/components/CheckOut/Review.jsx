...
export default function Review() {
  const {productToCheck,totalPrice,cardBadge,form,formPayment} = useContext(ProductsContext)
    ...
    <React.Fragment>
      ...
      <List disablePadding>
        {productToCheck.map((product) => (
          <ListItem key={product.id} sx={{ py: 1, px: 0 }}>
            ...
            <Typography variant="body2">{product.price} x {cardBadge}</Typography>
          </ListItem>
        ))}
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} sm={6}>
          ...
          <Typography gutterBottom>{`${form.address1},${form.city},${form.state},${form.zip},${form.country}`}</Typography>
        </Grid>
        <Grid item container direction="column" xs={12} sm={6}>
          ...
          { <Grid container>
              <React.Fragment key={`${formPayment.cardNumber}${formPayment.cardName}`}>
                <Grid item xs={6}>
